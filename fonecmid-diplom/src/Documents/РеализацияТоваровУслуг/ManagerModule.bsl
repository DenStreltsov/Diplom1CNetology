...
Процедура ВывестиТоварыУслуги(ДанныеДокументов, ТабличныйДокумент, Макет)
	...
	СуммаПрописью = НРег(ЧислоПрописью(ДанныеДокументов.СуммаДокумента, ФормСтрока, "Рубль,Рубля,Рублей,м,Копейка,Копейки,Копеек,ж,2"));
...
Процедура ВКМ_СоздатьДокументРеализации(ПоДоговору,Период) Экспорт
	
	НовыйДокумент = СоздатьДокумент();
	НовыйДокумент.Дата = ТекущаяДатаСеанса();
	НовыйДокумент.Заполнить(ПоДоговору);
	НовыйДокумент.ВКМ_ВыполнитьАвтозаполнение(Период);
